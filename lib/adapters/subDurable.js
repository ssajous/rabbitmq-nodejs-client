...
SubDurableAdapter.prototype._openCallback = function (exchange) {
  ...
  // Default to use ACK if not specified.
  var ackSettings = {};
  if (self.hub.options.ack === undefined || self.hub.options.ack === null) {
    ackSettings.ack = true;
  } else {
    ackSettings.ack = self.hub.options.ack;
  }
